...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        imm = self.model(input)
        mask = F.sigmoid(imm[:, :1])
        oimg = F.tanh(imm[:, 1:])
        mask = mask.repeat(1, 3, 1, 1)
        output = oimg * mask + input * (1 - mask)
        return output, oimg, mask
...
# mask gan 用的判别器
class TowerDiscriminator(nn.Module):
    def __init__(self):
        super(TowerDiscriminator, self).__init__()
